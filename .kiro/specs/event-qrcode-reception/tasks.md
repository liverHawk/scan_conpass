...
**説明**:
Connpass イベントページにアクセスして参加者一覧をスクレイピングで取得するサービスを実装します。HTML Rewriter ネイティブ API を使用してパース、フォールバックとして正規表現パースも対応。
...
4. HTML Rewriter を使用した高速ストリーミングパース方式が実装されている
5. 正規表現パース方式もフォールバックとして実装されている
